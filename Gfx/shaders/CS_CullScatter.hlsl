...
RWStructuredBuffer<MeshInstance> compactedInstanceBuffer : register(u5, space0);
...
[numthreads(64,1,1)]
void CS_CullScatter(uint3 tid : SV_DispatchThreadID)
{
    uint id = tid.x;
    if (id >= instanceCount)
        return;
    ...
    uint dst;
    InterlockedAdd(meshWriteCursor[inst.infoID], 1, dst);
    compactedInstanceBuffer[dst] = inst;
}
