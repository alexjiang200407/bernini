...
namespace
{
		...
		const bgl::SurfaceMaterialDesc& desc)
	{
		...
		{
			host.InvokeRender([handle = graph.preview, desc](editor::RenderContext& context) {
				try
				{
					context.scene.UpdateSurfaceMaterial(handle, desc);
				}
		...
		try
		{
			host.InvokeRender([&](editor::RenderContext& context) {
				graph.preview = context.scene.CreateSurfaceMaterial(desc);
			});
		}
		catch (const std::exception& e)
		{
		...
		if (previous.IsValid())
		{
			host.InvokeRender([previous](editor::RenderContext& context) {
				try
				{
					context.scene.DeleteMaterial(previous);
				}
				catch (const std::exception& e)
				{
		...
		MaterialPreviewWindow&   preview)
	{
		    ...
		    graph.preview.layerType == desc.layerType)
		{
			host.InvokeRender([handle = graph.preview, desc](editor::RenderContext& context) {
				try
				{
					context.scene.UpdateLoosePbrMaterial(handle, desc);
				}
				catch (const std::exception& e)
				{
		...
		// override completes before the previous material is released.
		host.InvokeRender([&](editor::RenderContext& context) {
			graph.preview = context.scene.CreateLoosePbrMaterial(desc);
		});
		for (const uint32_t submesh : graph.submeshes)
			preview.SetSubmeshMaterial(submesh, graph.preview);
