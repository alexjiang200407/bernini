...
TEST_CASE("An unchanged material is not read twice", "[materialeditor]")
{
	...
	REQUIRE(cached.Get(assetlib::AssetStore(sandbox.Root()), sandbox.Path()) != nullptr);
...
TEST_CASE("A different path is read even at the same stamp", "[materialeditor]")
{
	...
	REQUIRE(
		cached
			.Get(assetlib::AssetStore(sandbox.Root()), QString::fromStdWString(otherPath.wstring()))
			->name == "other");
}
...
TEST_CASE("A surface material opens as a graphless one", "[materialeditor][surface]")
{
	...
	const assetlib::BMaterial* material =
		cached.Get(assetlib::AssetStore(sandbox.Root()), sandbox.Path());
