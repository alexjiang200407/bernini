		...
		const ImportOptions& options)
	{
		...
		background::TaskResult result = background::RunWithLoadingScreen(
			...
			[&](background::Progress& progress) {
				...
				// pure assetlib. After the screen rather than behind it, this ran on the GUI thread
				// with nothing on screen saying the editor was still working, which reads as a hang.
				if (options.mesh)
				{
