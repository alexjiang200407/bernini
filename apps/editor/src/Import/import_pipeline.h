...
namespace editor
{
	 ...
	 * texture folder may not: `AssetStore::WriteTextures` names its output after the source's
	 * images, which an import cannot choose, so one already there is another import's and writing
	 * into it would overwrite that import's files.
	 *
