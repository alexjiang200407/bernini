...
TEST_CASE("A skinned mesh is placed in the skin's space, not the armature's", "[bmesh]")
{
	...
	REQUIRE(bmesh::InstanceTransform(mesh, 1) == glm::mat4(1.0f));
}
...
TEST_CASE("A static attachment still hangs off the bone it is parented to", "[bmesh]")
{
	...
	REQUIRE(bmesh::InstanceTransform(mesh, 2) == bmesh::WorldTransform(mesh, 2));
	REQUIRE(bmesh::InstanceTransform(mesh, 2) != glm::mat4(1.0f));
}
...
TEST_CASE("Only the skinned mesh's own node is exempt", "[bmesh]")
{
	...
	REQUIRE(bmesh::InstanceTransform(mesh, 0) == bmesh::WorldTransform(mesh, 0));
}
