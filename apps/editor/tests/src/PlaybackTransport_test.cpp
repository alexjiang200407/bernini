...
TEST_CASE("Normalized scrubbing is the inverse of the position, in either domain", "[animation]")
{
	auto transport = Loaded(/*loop*/ true);

	// The clip's own domain: 0.3 s of period across the whole bar.
	transport.ScrubNormalized(0.5f);
	CHECK(transport.GetTimeSeconds() == Catch::Approx(0.15f));
	CHECK(transport.GetNormalizedPosition() == Catch::Approx(0.5f));

	// Out of range is clamped to the domain's ends rather than wrapped around them.
	transport.ScrubNormalized(-1.0f);
	CHECK(transport.GetTimeSeconds() == 0.0f);

	// The far end of a *looping* clip is its start: the clamp lands on the period, and the period is
	// where the clip wraps. A timeline dragged fully right therefore shows frame 0 -- the same pose
	// as the last frame -- which is why SyncTransportUi will not write the bar back mid-drag.
	transport.ScrubNormalized(4.0f);
	CHECK(transport.GetTimeSeconds() == 0.0f);
	CHECK(transport.GetNormalizedPosition() == 0.0f);

	// A one-shot has a far end to stand on, and stands on it.
	auto once = Loaded(/*loop*/ false);
	once.ScrubNormalized(4.0f);
	CHECK(once.GetTimeSeconds() == Catch::Approx(0.3f));
	CHECK(once.GetNormalizedPosition() == Catch::Approx(1.0f));

	// And the window's, where the same fraction means a different clock.
	transport.SetTransitionWindow(10.0f, 12.0f);
	transport.ScrubNormalized(0.25f);
	CHECK(transport.GetTimeSeconds() == Catch::Approx(10.5f));
	CHECK(transport.GetNormalizedPosition() == Catch::Approx(0.25f));

	// Round-trips: what a timeline paints is what scrubbing it back gives.
	for (const float at : { 0.0f, 0.125f, 0.6f, 1.0f })
	{
		transport.ScrubNormalized(at);
		CHECK(transport.GetNormalizedPosition() == Catch::Approx(at));
	}
}

TEST_CASE("Normalized scrubbing is inert with no clips", "[animation]")
{
	auto transport = PlaybackTransport();
	transport.ScrubNormalized(0.5f);
	CHECK(transport.GetTimeSeconds() == 0.0f);
