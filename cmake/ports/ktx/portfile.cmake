# Overlay of vcpkg's ktx port, forked ONLY to add 0007-basisu-job-pool-deadlock.patch.
#
# basisu's job_pool destructor -- vendored inside ktx at external/basisu -- notified its workers
# without holding the mutex they wait under, so a worker could miss the kill notify and sleep
# forever while the destructor's join() waited on it. It wedged an assetlib_tests shard for 83
# minutes at no CPU, holding the machine-wide suite lock; docs/known_issues.md carries the entry.
#
# Everything else here is vcpkg's port verbatim, at the version its baseline pins. Upstream
# basis_universal already takes the lock, so the patch will stop applying once ktx vendors a newer
# basisu -- and vcpkg fails the build when a patch does not apply, which is how we find out. That
# is the point to delete this directory; the overlay-ports entry in vcpkg-configuration.json names
# the parent and stays while any port still lives there.
...
vcpkg_from_github(
    ...
    HEAD_REF master
    PATCHES
        0007-basisu-job-pool-deadlock.patch
