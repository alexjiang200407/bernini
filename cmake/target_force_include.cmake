...
function(target_force_include TARGET_NAME)
    ...
    if(ARG_PRIVATE)
        foreach(HEADER ${ARG_PRIVATE})
            ...
            elseif(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
                target_compile_options(${TARGET_NAME} PRIVATE "SHELL:-include \"${HEADER}\"")
            else()
    ...
    if(ARG_PUBLIC)
        foreach(HEADER ${ARG_PUBLIC})
            ...
            elseif(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
                target_compile_options(${TARGET_NAME} PUBLIC "SHELL:-include \"${HEADER}\"")
            else()
                message(WARNING "Force include not implemented for this compiler")
            endif()
        endforeach()
    ...
    if(ARG_INTERFACE)
        foreach(HEADER ${ARG_INTERFACE})
            ...
            elseif(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
                target_compile_options(${TARGET_NAME} INTERFACE "SHELL:-include \"${HEADER}\"")
            else()
                message(WARNING "Force include not implemented for this compiler")
            endif()
        endforeach()
