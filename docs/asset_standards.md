...
(CPU) mirror [libs/bgl_common/idl/src/VertexLayout.slang](libs/bgl_common/idl/src/VertexLayout.slang) (GPU) — the enum
...
* **`.bmaterial`** — **a shading-model tag plus that model's parameters**, as an authored text
  ...
  **Adding a shading model** means: a `ShadingModel` enumerator, a payload struct, its document
  ...
  material struct in the IDL (`libs/bgl_common/idl/src`, regenerated by `just idl`), whose texture handles
  ...
  (`libs/bgl_common/idl/src/PsoType.slang`) appended at the end, because `c_Psos` in `ForwardPass.cpp` is
