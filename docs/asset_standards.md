...
* **`.bmaterial`** — **a shading-model tag plus that model's parameters**, as an authored text
  ...
  **Adding a shading model** means: a `ShadingModel` enumerator, a payload struct, its document
  ...
  material struct in the IDL (`libs/bgl/idl/src`, regenerated by `just idl`), whose texture fields
  are `Entry<TextureHandle>` into the scene's texture table and never a handle inline — a payload
  holding one cannot be read out of a raw buffer at all on Metal (see
  [Slang Shaders](docs/slang_shaders.md)); an `EntryBuffer<XMaterial>` field in
  `forward/MaterialData.slang`, the `extension` conforming the
  struct to the shading interfaces, and the `c_MaterialBuffers` row in `ForwardPass.cpp` that binds
  that field by uniform key; pixel-shader modules under `libs/bgl/shaders/src` for the opaque,
  alpha-test and hashed modes — blend instead shares `Forward_Transparent` across material types,
  dispatching on the boolean `materialIsLoose` varying the mesh stage stamps from `IsLoosePso`
  (`forward/common.slang`), and a third material type outgrows that boolean; `PsoType` enumerators
...
flowchart TD
    ...
    subgraph SCENE["Scene (runtime)"]
        ...
        AT["AddTextureAsset (bindless SRV + a texture-table entry)"]
        CM["CreatePbrMaterial (table entries → idl::PbrMaterial)"]
    end
