...
| `.bavatar` | One rig's authored half: named body `parts` (single bones or inclusive start/end chains), foot-plant `legs`, and per-clip `plant` weights. Parts use exact bone names and optional common/custom role names; see [Named body parts](docs/skinning.md#named-body-parts). A zero plant weight excludes a clip; the legacy `unplanted` list still reads. Found by convention from the `.bskel` (`avatarKeyFor`) — the path is the attachment. Its struct is [avatar.h](libs/assetlib/include/assetlib/avatar.h). |
