...
* **The Slang copy is verbatim apart from Metal padding.** MSL rounds a struct's size up to its
  alignment where the C/C++ and DXIL rules do not, so `Mesh` (a `float4x4` plus a `RangeWithCount`)
  packs to 72 bytes for them but wants an 80-byte stride there. Left alone, element 1 of the buffer
  is read from the wrong offset on that backend only, and silently. idlgen reflects a third, Metal
  session for exactly this and appends a `pad` member to **both** outputs — the padding has to be a
  member of the shared struct, because padding either mirror alone would leave the other target on
  the narrower stride.
  ...
  **Trailing padding cannot fix an interior mismatch.** MSL aligns a vector to its own width where
  ...
  `LoosePbrMaterial` lead with `baseColorFactor`. A 3-component vector member is refused outright
  for the same reason — 12 bytes to the C/C++ rules, 16 to MSL, so everything after it moves.
