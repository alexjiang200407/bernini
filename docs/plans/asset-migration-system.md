...
- **ADR-9 — The schema table is chunk id 0**, and each chunk entry names its element type by index
  ...
  learn separately.*
- **ADR-10 — The schema is its own library, `libs/schema`**, that links `core` only and knows
  nothing of assets: a tool that wants to read a file's schema should not have to link the codec —
  KTX, meshoptimizer, tinygltf — to do it, and nothing in it is asset-specific, so it is not named
  as if it were (it was `assetlib_schema` until review of #390). The registrations of the asset
  PODs are `assetlib`'s `AssetSchemaBuilder`, beside the io that writes them. *Rejected: inside
  `assetlib` (the plan's first answer, changed on review of #386); `core`, for a `bgl` or editor
...
- **ADR-12 — Conversion rules.** Same name and kind: copy. Integer or enum widening: convert.
  ...
  schema: its declared default. Only in the file: dropped. A field declared with its old name
  (`AddRenamedField`): matched under either. Anything else — narrowing, float↔integer, struct↔scalar — is ADR-6's error. *Rejected:
