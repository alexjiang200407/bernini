...
`--taa`, `--width`/`--height`, `--render-scale` and `--taa-sharpness` are the framing the numbers are
read at — the last two are `RenderTargetDesc::renderScale` and `taaSharpness`, and
[Temporal Antialiasing](taa.md) § Render scale holds a table the scale took.
