...
accumulated. Each viewport reads its own `temporalAA` key in `config.json` — under `materialEditor` and
`animationEditor`, beside the other things that size that window's render target, rather than
...
it was reported on. `renderScale` under `materialEditor` and `animationEditor` in `config.json`
sets the value the editor starts at; the menu moves every viewport
from there, live, because the comparison is what shows a temporal artifact.

Beside it is **TAA Reconstruction Width** (`taaReconstructionWidth`, same two keys, 0.4 by
...
animal-run's `TownStreet` lit by its own environment, at `8e6f469d` -- an engine that still drew the
static scene a second time, depth-only, for the blob-shadow receiver (the StaticDepth row):
...
| StaticDepth (depth-only redraw) | 0.11 | 0.10 | 0.10 | 1.11 | 1.09 |
...
stopped. The forward pass on `TownStreet` moves by an eighth for a 56% cut in pixels, and the depth-only
redraw not at all — both are geometry-bound there, and a render scale does nothing for geometry.
...
* **A change *no motion vector describes* drops the accumulation.** Reprojection answers "where was
  ...
  release, an environment map or skybox, the scene's ground plane, a blob shadow set or cleared, a
  placement hidden or unhidden. Anything a caller moves every frame (the
...
* **The clamp reads the 3×3, not the cross the dilation reads.** A five-tap cross clamp was built,
  measured and judged by eye on a hashed hair asset, then taken out. It made the pass 21–23% cheaper
  on the M3 Pro (0.56 ms at 4K), and on opaque edges it was tighter: pan trail 0.0065 → 0.0037,
  animating outline 2.2e-4 → 1.6e-4. On stochastic coverage five samples of a two-population field
  give a noisy σ. A converged hashed patch flickered about 13× as much (0.0016 → 0.0205 at rest,
  0.0037 → 0.044 under a pan), the hashed ramp's smear tripled, and the hair visibly shimmered. It
  would suit a project with little stochastic coverage, which is why it is worth keeping as a
  measurement rather than a default.

* **The sharpen is FSR 1's RCAS, where FSR 2 puts it.** A five-tap cross whose negative lobe is
  solved per pixel for the most sharpening that cannot push the centre past its neighbours' range,
  with FSR 2's noise detection on (`lib/math/rcas.slang`, after `ffx_fsr1.h` `FsrRcasF`). It runs on
  the resolved linear HDR, exposure already folded in, ahead of bloom and the display curve, exactly
  where FSR 2's RCAS pass reads its accumulation. It lives in `PostProcess` rather than as a pass of
