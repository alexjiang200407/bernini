...
{
	void
	CheckImportRoundTrip(const std::filesystem::path& glb)
	{
		namespace fs = std::filesystem;
		...
		REQUIRE_FALSE(
			imported.materials.empty());  // the glTF has them; the import must not carry them
		...
		SECTION("the file set is the categories and nothing else")
		{
			...
			// Neither source carries a skin, so WriteImportedRig writes no rig.
...
{
	CheckImportRoundTrip(assetlib::test::TexturedGltfPath());
}

TEST_CASE("a full-size import lands in the project's categories and reads back", "[.fullimport]")
{
	CheckImportRoundTrip("assets/apples.glb");
}
