...
namespace bgl
{
	 ...
	 * one command allocator per slot, plus present and resize.
	 *
	 ...
	 * GraphicsBase.
	 *
	 * Slot indices run to `c_SwapchainImageCount`; passing anything else is a precondition
	...
	public:
		...
		/** The slot the next frame records into. */
		[[nodiscard]] virtual uint32_t
		FrameSlot() const noexcept = 0;

		/** The slot the last presented frame landed in; what a readback must sample. */
		[[nodiscard]] virtual uint32_t
		LastPresentedSlot() const noexcept = 0;

		[[nodiscard]] virtual bool
		IsHeadless() const noexcept = 0;

		/** Zero when no frame has used the slot yet, so nothing needs waiting on. */
		[[nodiscard]] virtual uint64_t
		SlotFence(uint32_t slot) const noexcept = 0;

		virtual void
		SetSlotFence(uint32_t slot, uint64_t fenceValue) noexcept = 0;

		/** Borrowed; the target owns it. Reset it before recording into the slot. */
		[[nodiscard]] virtual ICommandAllocator*
		SlotAllocator(uint32_t slot) const noexcept = 0;

		[[nodiscard]] virtual TextureHandle
		BackbufferTexture(uint32_t slot) const noexcept = 0;

		[[nodiscard]] virtual RtvHandle
		BackbufferRtv(uint32_t slot) const noexcept = 0;

		[[nodiscard]] virtual DsvHandle
		DepthDsv() const noexcept = 0;

		/**
		 * Presents the frame just submitted, then advances to the slot the next one records into.
		 * A headless target presents nothing and advances round-robin; a windowed one takes the
		 * slot back from its swapchain.
		 *
		 * @pre the frame's command list has been submitted and its fence recorded through
		 *      SetSlotFence, or the presented backbuffer may still be being written.
		 */
		virtual void
		 ...
		 * frame ring: every slot fence returns to zero, since the slots they described are gone.
		 *
