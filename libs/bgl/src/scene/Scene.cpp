...
namespace bgl
{
		...
		const FootPlantDesc&          footPlant)
	{
		try
		{
			...
			// Four bytes to a uint, least-significant first. The tail of the last word is zero,
			// which is a leg that is never planted -- the same thing an absent weight would mean.
		...
		const FootPlantDesc&            footPlant)
	{
		...
		return AttachSkinnedRecords(base, skeleton, animations, footPlant);
	}
	...
	Scene::DeleteGeom(GeomHandle geom)
	{
		...
		if (record.skinnedGeom)
		{
			...
			if (!skinned.legs.Null())
			{
				m_SkinnedLegs.EraseByIndex(skinned.legs.range.offsetStart);
				m_PlantWeights.EraseByIndex(skinned.plantWeights.offsetStart);
			}
