...
//
// Driven through IResourceManager rather than a backend's concrete type, because the invariants are
// the seam's and not D3D12's: on Metal the number is a pool slot the dispatch rewrite looks up, on
// D3D12 a descriptor index, and neither may alias or move.
...
TEST_CASE("Live resources on the shared heap never alias a bindless index", "[descriptor]")
{
	...
	auto manager = gfxBase->GetResourceManagerCpy();

	// Buffers and SRV textures share one shader-visible heap on D3D12, so uniqueness must hold
	// across both kinds at once, not per kind.
...
TEST_CASE("A resource's bindless index is stable for its lifetime", "[descriptor]")
{
	...
	manager->DestroyBuffer(doomed, false);
