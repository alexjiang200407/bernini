...
namespace
{
	...
	bgl::RenderTargetRef
	HeadlessTarget(const bgl::RenderContextRef& ctx)
	{
	...
	assetlib::ImageData
	PollUntilResolved(const bgl::RenderContextRef& ctx, bgl::CaptureTicket ticket)
	{
		while (true)
		{
			if (auto image = ctx->TryResolveCapture(ticket); image.has_value())
...
TEST_CASE(
	"A split-phase capture resolves to the frame a blocking screenshot returns",
	"[capture][rendercontext]")
{
	...
	auto target = HeadlessTarget(ctx);
	...
	ctx->DrawFrame(target, CubeJob(view));
	ctx->DrawFrame(target, CubeJob(view));

	const bgl::CaptureTicket ticket = ctx->SubmitCapture(target);
	REQUIRE(ticket.IsValid());

	const assetlib::ImageData resolved = PollUntilResolved(ctx, ticket);

	// The ticket was spent by the resolve that returned the image.
	REQUIRE_THROWS_AS(ctx->TryResolveCapture(ticket), bgl::GraphicsError);

	const assetlib::ImageData blocking = ctx->ScreenshotToMemory(target);
...
TEST_CASE("Capture tickets are bounded, single-spend, and discardable", "[capture][rendercontext]")
{
	auto gfx = bgl::CreateGraphics(CaptureOptions());
	REQUIRE(gfx != nullptr);
	...
	REQUIRE_THROWS_AS(ctx->SubmitCapture(target), bgl::GraphicsError);
	ctx->EndFrame();

	REQUIRE_THROWS_AS(ctx->TryResolveCapture(bgl::CaptureTicket{}), bgl::GraphicsError);
	REQUIRE_NOTHROW(ctx->DiscardCapture(bgl::CaptureTicket{}));

	// Fill every slot; one more must refuse rather than silently block or drop a capture.
	std::vector<bgl::CaptureTicket> tickets;
	for (uint32_t i = 0; i < bgl::IRenderContext::c_MaxPendingCaptures; ++i)
		tickets.push_back(ctx->SubmitCapture(target));
	REQUIRE_THROWS_AS(ctx->SubmitCapture(target), bgl::GraphicsError);

	// A discard frees its slot for the next submit, and discarding twice is a no-op.
	ctx->DiscardCapture(tickets.front());
	REQUIRE_NOTHROW(ctx->DiscardCapture(tickets.front()));
	REQUIRE_THROWS_AS(ctx->TryResolveCapture(tickets.front()), bgl::GraphicsError);

	const bgl::CaptureTicket refilled = ctx->SubmitCapture(target);
	REQUIRE(refilled.IsValid());

	// Drain what is still live so teardown finds no surprises.
	for (uint32_t i = 1; i < tickets.size(); ++i) PollUntilResolved(ctx, tickets[i]);
	PollUntilResolved(ctx, refilled);
}
