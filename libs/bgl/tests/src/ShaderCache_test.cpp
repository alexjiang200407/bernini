...
// Compilation dominates a cold start, and a client that shows a startup screen for it can only
// size the bar from what bgl says it is about to do. Two things have to hold for that bar to be
// honest, and neither is visible from outside: the total is fixed before the first report, and it
// is exactly the number of reports that follow.
TEST_CASE(
	"Graphics reports every pipeline it builds, against a total fixed up front",
	"[shadercache][render]")
{
	auto reports = std::vector<std::pair<uint32_t, uint32_t>>();
	auto names   = std::vector<std::string>();

	auto opts               = bgl::GraphicsOptions();
	opts.enableDebugLayer   = true;
	opts.onPipelineProgress = [&](const bgl::PipelineProgress& progress) {
		reports.emplace_back(progress.done, progress.total);
		names.emplace_back(progress.name);
	};
	...
	const uint32_t total = reports.front().second;

	// A bar sized from the first report and filled by the ones after it: the total never moves,
	// every pipeline is counted once, and the last report is the last pipeline.
	CHECK(reports.size() == total);
	for (uint32_t i = 0; i < reports.size(); ++i)
	{
		INFO("report " << i << " named " << names[i]);
		CHECK(reports[i].first == i);
		CHECK(reports[i].second == total);
		CHECK_FALSE(names[i].empty());
	}

	// Every one of them says what it is, so the screen shows a shader rather than a number.
	auto unique = names;
