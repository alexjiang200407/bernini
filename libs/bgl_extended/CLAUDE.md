...
- The renderer's PSOs are built together, in parallel: a pass's `Init` requests its always-on
  ...
  meshlet kernels are the exception: `RenderContext::EnsureBucketPipelinesExist` builds each bucket in
...
- Slang sessions are per thread (`src/slang/SlangSessions.h`): a thread's first compile creates
  ...
  start-up build's in `CreateGraphics`, a demand build's in `EnsureBucketPipelinesExist` — because each
