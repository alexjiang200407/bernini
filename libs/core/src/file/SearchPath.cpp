...
{
	void
	SearchPath::Mount(std::shared_ptr<IFileSystem> mount)
	{
		if (mount != nullptr)
	...
	void
	SearchPath::SetMask(std::span<const std::string> paths)
	{
		m_Mask.clear();
	...
	bool
	SearchPath::IsMasked(std::string_view path) const noexcept
	{
		return m_Mask.contains(path);
	}

	const IFileSystem*
	SearchPath::Resolve(std::string_view path) const noexcept
	{
		if (IsMasked(path))
			return nullptr;
	...
	bool
	SearchPath::IsPathReadOnly(std::string_view path) const noexcept
	{
	...
	bool
	SearchPath::Exists(std::string_view path) const noexcept
	{
		return Resolve(path) != nullptr;
	}

	std::optional<FileStamp>
	SearchPath::Stat(std::string_view path) const noexcept
	{
		const IFileSystem* mount = Resolve(path);
	...
	std::vector<std::byte>
	SearchPath::Read(std::string_view path) const
	{
		const IFileSystem* mount = Resolve(path);
		if (mount == nullptr)
			core::throw_runtime_error("SearchPath: no mount carries '{}'", path);

		return mount->Read(path);
	}

	std::vector<std::byte>
	SearchPath::ReadRange(std::string_view path, uint64_t offset, uint64_t size) const
	{
		const IFileSystem* mount = Resolve(path);
		if (mount == nullptr)
			core::throw_runtime_error("SearchPath: no mount carries '{}'", path);
	...
	SearchPath::Enumerate(std::string_view prefix) const
	{
	...
	bool
	SearchPath::IsReadOnly() const noexcept
	{
