...
namespace editor
{
	 ...
	 * @param store Project store, or an explicit external store for a configured environment.
	 ...
	 */
	[[nodiscard]] EDITOR_SDK_EXPORT AppliedEnvironment
	ApplyEnvironment(
		bgl::IScene*                scene,
		bgl::ISceneView*            view,
	 ...
	 */
	[[nodiscard]] EDITOR_SDK_EXPORT QStringList
	 ...
	 */
	[[nodiscard]] EDITOR_SDK_EXPORT std::optional<std::string>
	 ...
	 * Must be called on the render thread.
	 *
	 * @param store Store that owns the environment and its referenced maps.
	 * @throws bgl::SceneError if a displaced handle was already deleted.
	 */
	EDITOR_SDK_EXPORT void
	BindEnvironment(
