...
def cpp_args_for(module, public, internal, tool):
    """The --cpp-out-dir/--namespace flags for one module (empty = shader-only, nothing to generate)."""
...
def main():
    ...
    for module in modules:
        ...
        cpp_args = cpp_args_for(module, public, internal, tool)
        if not cpp_args:
            print(f"[shader-only] {rel}")
            continue
        cmd = [
            tool,
            "--src-root", SRC_ROOT,
