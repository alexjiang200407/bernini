...
class TestModuleOf:
    ...
    def test_a_backend_counts_as_its_library(self):
        assert count_source.module_of('libs/bgl_extended/src/d3d12/Device.cpp') == 'bgl_extended'
        assert count_source.module_of('libs/bgl_extended/shaders/forward.slang') == 'bgl_extended'
...
class TestCollect:
    def test_the_two_breakdowns_partition_the_same_files(self, tmp_path):
        ...
        write(root, 'libs/bgl_extended/src/Device.cpp', 10)
        write(root, 'libs/bgl_extended/shaders/forward.slang', 5)
        write(root, 'libs/bgl_extended/tests/DeviceTests.cpp', 7)
    ...
    def test_a_test_file_is_charged_to_its_module_as_a_test(self, tmp_path):
        """The src/test split survives the module dimension: bgl_extended's tests are bgl_extended's, not a module of
        their own."""
        root = str(tmp_path)
        write(root, '.gitignore', 0)
        write(root, 'libs/bgl_extended/src/Device.cpp', 10)
        ...
        assert by_module['bgl_extended']['src'] == {'files': 1, 'lines': 10}
        assert by_module['bgl_extended']['test'] == {'files': 1, 'lines': 7}
